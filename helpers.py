...
def visualize_data(dataset_dir, show_masks=True, max_samples=5, shuffle=True):
        ...
        if file_name.startswith("photo") and cnt < max_samples:
            ...
            if show_masks:
                with rasterio.open(mask_path) as mask_ds:
                    ...
                    legend_handles = [mpatches.Patch(color=SurfaceMapping().mapping_names_colors[value][1], label=SurfaceMapping().mapping_names_colors[value][0]) for value in unique_values]
